...
def start_cameras(camera_ids):
    ...
    for camera_id in camera_ids:
        p = multiprocessing.Process(target=camera_process, args=(camera_id,))
        p.start()
        process.append(p)
...
if __name__ == "__main__":
    camera_count = int(input("📷 How many cameras do you want to start? "))
    camera_ids = list(range(camera_count))
    start_cameras(camera_ids)